...
updatecompany()
{
   setLayout(null);
    ...
    try
    {
        ...
        pst=c1.prepareStatement("select name from company");
        rs=pst.executeQuery();
        while(rs.next())
        {
...
public void actionPerformed(ActionEvent ae)
{
    if(ae.getSource()==bt1)
    {
        ...
        if(r.equals(""))
        {
            JOptionPane.showMessageDialog(this,"please enter contact to be update");
        }
        else
        {
            pst=c1.prepareStatement("update company set contact=? where name=?");
            pst.setString(1,r);
            pst.setString(2,p);
        ...
        if(u.equals(""))
        {
            JOptionPane.showMessageDialog(this,"please enter email to be update");
        }
        else
        {
            pst=c1.prepareStatement("update company set email=? where name=?");
            pst.setString(1,u);
            pst.setString(2,o);
