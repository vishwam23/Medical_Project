...
Newsales()
{
    ...
    try
    {
        ...
        pst=c1.prepareStatement("select name,company from makku");
        rs=pst.executeQuery();
        while(rs.next())
        {
...
public void actionPerformed(ActionEvent ae)
{
    if(ae.getSource()==bt1)
    {
        try
        {
            ...
            {
            pst=c1.prepareStatement("insert into newsales values(?,?,?,?,?,?,?,?)");
            pst.setString(1,a);
            pst.setString(2,b);
            pst.setString(3,c);
            pst.setString(4,d);
            ...
            if(j>0)
            {
                pst=c1.prepareStatement("Select * from newsales where productname=?");
                pst.setString(1,a);
                rs=pst.executeQuery();
                if(rs.next())
                {
                    ...
                    pst=c1.prepareStatement("Update newsales set productquantity=? where productname=?");
